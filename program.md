# autoresearch-seg: Agent Instructions for Segmentation Research
...
1. **Agree on a run tag**: propose a tag based on today's date (e.g. `apr20`). The branch `autoresearch-seg/<tag>` must not already exist — this is a fresh run.
2. **Create the branch**: `git checkout -b autoresearch-seg/<tag>` from current master.
